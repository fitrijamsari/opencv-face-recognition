...
def markAttendance(name):
    with open("attendance.csv", "r+") as f:
        ...
        for line in myDataList:
            entry = line.split(",")  # split data entry by ,
            nameList.append(entry[0])  # entry 0 is name, append on list

            # if name not in list, then we find the time
...
while True:
    ...
    # itterate all faces in current frame, then compare with the encoding we found before
